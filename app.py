...
with st.sidebar:
    ...
    armm_label = "Autonomous Region in Muslim Mindanao (ARMM)"
    if armm_label not in available_regions:
        available_regions.append(armm_label)
...
if st.button("Generate Development Strategy", key="deploy_btn"):
    ...
    # Show a notice if ARMM is selected
    if selected_region == armm_label:
        st.info("📌 Note: ARMM data was limited during training. The system is providing a generalized baseline forecast for this region.")
    ...
    # Map Region safely (If ARMM, it skips this and uses baseline 0.0)
    ...
    try:
        # treat the model output as the 'Projected Increase'
...
st.divider()
st.caption("Final Model Deployment - CRISP-DM Phase 6 - Housing Estate Market Potential")
